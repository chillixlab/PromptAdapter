...
def test_JSONリポジトリからサンプル定義を読み込める() -> None:
    # Given: 接続定義JSONとモデル定義JSONを読み込むリポジトリを用意する
    ...
    assert (
        model_repository.get("gpt41_openai").litellm_mode == AIModelCallMode.RESPONSES
    )


def test_存在しない接続名を指定すると例外になる() -> None:
    # Given: 接続定義JSONを読み込むリポジトリを用意する
    connection_repository = CsvConnectionRepository(
        FIXTURE_DIR / "model_connections.example.json"
    )
